...
def _parse_summary(soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    panel_child = None
    # Now find the w3-half div that has a w3-panel child
    for w3_half_div in w3_half_divs:
        # Check if this w3-half div has a child with class 'w3-panel'
        panel_child = w3_half_div.find("div", class_="w3-panel")
        if panel_child:
            break
...
def _parse_high_level_widgets(soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    logger.debug("=== Assets & Liabilities Section ===")
    if assets_h3:
        logger.debug("Found h3 with 'Assets & Liabilities':")
        logger.debug(f"H3 element: {assets_h3}")
        logger.debug(f"H3 text: {assets_h3.get_text(strip=True)}")

        # Get parent div
        parent_div = assets_h3.find_parent('div')
        if parent_div:
            logger.debug(f"Parent div: {parent_div}")
            logger.debug(f"Parent div text: {parent_div.get_text(strip=True)}")
            ...
            if table:
                logger.debug("Found table in parent div")
                    ...
                    if len(cells) >= 2:
                        ...
                        if "Assets" in category_text:
                            ...
                            if span_element:
                                ...
                                logger.debug(f"Assets description: {result['assets_description']}")
                            ...
                            if span_element:
                                ...
                                logger.debug(f"Liabilities description: {result['liabilities_description']}")
            else:
                logger.debug("No table found in parent div")
        else:
            logger.debug("No parent div found")
    else:
    ...
    if education_h3:
        ...
        logger.debug(f"H3 element: {education_h3}")
        logger.debug(f"H3 text: {education_h3.get_text(strip=True)}")
        ...
        if parent_div:
            ...
            if category_match:
                ...
                logger.debug(f"Education category: {result['education_category']}")
            ...
            if br_match:
                ...
                logger.debug(f"Education details: {result['education_details']}")
                ...
                if len(parts) > 1:
                    ...
                    if lines:
                        if not result["education_category"]:
                            ...
                            logger.info(f"Education category (fallback): {result['education_category']}")
                        
                        if len(lines) > 1 and not result["education_details"]:
                            result["education_details"] = ' '.join(lines[1:]).strip()
                            logger.info(f"Education details (fallback): {result['education_details']}")
        else:
            logger.debug("No parent div found")
    else:
        logger.info("No h3 with 'Educational Details' found")
    ...
    # Criminal cases: match variations like "Number of Criminal Cases", "Criminal Cases"
    ...
    # Assets summary: match "Assets", "Total Assets" followed by Rs or ₹
...
def scrape_candidate(
    ...
    widgets = _parse_high_level_widgets(soup)
...
def main() -> None:
    ...
    logger.info(f"Summary and tables written to {args.out}")
    if args.tables_dir:
        write_tables(data, args.tables_dir)
        logger.info(f"Detailed tables written to directory {args.tables_dir}")
